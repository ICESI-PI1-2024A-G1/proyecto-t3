    ...
    try:
        for malla_anterior in MallaCurricular.objects.filter(
            ...
            for curso_anterior in Curso.objects.filter(
                ...
                for grupo_anterior in GrupoDeClase.objects.filter(clase__curso=curso_anterior).distinct():
                    grupo = GrupoDeClase.objects.create()
                    for clase_anterior in Clase.objects.filter(grupo_clases=grupo_anterior).distinct():
